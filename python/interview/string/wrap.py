def wrap(text, num_of_chars):
    if len(text) == 0:
        raise ValueError
    wl = {len(w): w for w in text.split()}
    if not all(len(word) < num_of_chars for word in text.split()):
        max_length = max(len(word) for word in text.split())
        raise ValueError("num_of_chars shoud be greater than {} : {}".format(max_length + 1, wl[max_length]))

    new_text=""
    length = 0
    for word in text.split():
        if length + 1 + len(word) > num_of_chars:
            new_text += "\\n"
            length = 0

        new_text += " " + word
        length += 1 + len(word)
    assert all(len(word) < num_of_chars for i in new_text.splitlines())
    return new_text
